...
public class CatalogFlowTest {
    ...
    void searchShowsMatchingCards(){
        ...
        assertEquals(1,cards.size(),"headphones search should return one classroom products");

        for (WebElement card:cards){
            String title=card.findElements(PRODUCT_TITLE).getFirst().getText();
            assertAll(
    ...
    void searchShowEmptyStateForUnknownProduct(){
    ...
    @Test
    @DisplayName("Sort dropdown re-renders cards ans prices become ascending")
    ...
    void hoverRevealQuickViewMarker(){
    ...
    @Test
    @DisplayName("Catalog page title is displayed correctly")
    void verifyPageTitle() {
        assertEquals("Product Catalog | SDET Retail Automation Lab", driver.getTitle());
    }

    @Test
    @DisplayName("Product has price")
    void firstProductHasPrice(){
        String price=driver.findElements(PRODUCT_PRICE).getFirst().getText();
        assertFalse(price.isBlank());
    }
    @Test
    @DisplayName("Product has name")
    void firstProductHasName(){
        String title=driver.findElements(PRODUCT_TITLE).getFirst().getText();
    ...
    @Test
    void searchBoxAcceptsInput() {

        WebElement search = driver.findElement(SEARCH_INPUT);
        search.sendKeys("headphones");
        assertEquals("headphones", search.getAttribute("value"));
    }
